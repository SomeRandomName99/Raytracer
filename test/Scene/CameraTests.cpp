...
TEST(camera_tests, horizontalCanvas){
  ...
  EXPECT_FLOAT_EQ(c.pixelSize_, 0.01f);
}
...
TEST(ray_construction_tests, rayThroughCornerOfCanvas){
  ...
  EXPECT_EQ(r.origin_, utility::Point(0,0,0));
  EXPECT_EQ(r.direction_, utility::Vector(0.665186426f, 0.332593213f, -0.6685123582f));
}
...
TEST(rendering_tests, renderingWorldWithCamera){
  ...
  EXPECT_EQ(image.pixelAt(5,5), utility::Color(0.380661107f, 0.4758263837f, 0.28549583f));
}